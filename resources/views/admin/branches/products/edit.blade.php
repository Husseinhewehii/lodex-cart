        ...
        <section class="section">
            ...
            <div class="page-header">
                ...
                <ol class="breadcrumb">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{trans('update_branch_products')}} #{{ $branchProduct->id }}</li>
                </ol>
            </div>
            <!--page-header closed-->
                    ...
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-header">
                                <h4>{{trans('update_branch_products')}}</h4>
                            </div>
                            <div class="card-body">
                                @include('admin.errors')

                                @section("open-form-tag")
                                    <form action="{{ route('admin.branch.products.update', ['branch'=>$branchProduct->branch_id,'branchProduct' => $branchProduct]) }}" method="post">
                                @show
                                    @method("PUT")
                                    @csrf
                                        ...
                                        <div class="form-group overflow-hidden">
                                            ...
                                            <option value="">{{ trans('select_category') }} *</option>
                                                @foreach($categories as $category)
                                                    <option value="{{ $category->id }}" {{ $category->id == old( 'category_id', $branchProduct->category_id) ? "selected":null }}>{{ $category->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                    {{--<div class="form-group col-md-4" id="products" >--}}
                                        {{--<div class="form-group overflow-hidden">--}}
                                            ...
                                            {{--<select name="product_id"  class="form-control select2 w-100" id="product_id" >--}}
                                                ...
                                                {{--@foreach($products as $product)--}}
                                                    {{--<option value="{{ $product->id }}" {{ $product->id == old( 'product_id', $branchProduct->product_id) ? "selected":null }}>{{ $product->name }}</option>--}}
                                                {{--@endforeach--}}
                                            {{--</select>--}}
                                        {{--</div>--}}
                                    {{--</div>--}}
                                    <div class="form-group col-md-4">
                                        ...
                                        <input type="text" class="form-control" value="{{ old('price',$branchProduct->price) }}" name="price" id="price" >
                                    </div>
                                    <div class="form-group col-md-4">
                                        <label for="discount">{{ trans('discount') }} %</label>
                                        <input type="number" min="0" class="form-control" value="{{ old('discount',$branchProduct->discount) }}" name="discount" id="discount" >
                                    </div>
                                    <div class="form-group col-md-4">
                                        ...
                                        <div class="input-group date">
                                            ...
                                            <input type="text" class="form-control pull-right" name="discount_till" id="discount_till" value="{{ old('discount_till', $branchProduct->discount_till) }}" placeholder="{{ trans('discount_till') }}">
                                        </div>
                                    </div>
                                    <div class="form-group col-md-3">
                                        <label class="custom-switch">
                                            <input type="checkbox" name="active" value="1" {{ $branchProduct->active ? 'checked' : '' }} class="custom-switch-input">
                                    ...
                                    {{--<div class="form-group col-md-3">--}}
                                        {{--<label class="custom-switch">--}}
                                            {{--<input type="checkbox" name="wholesale" value="1" {{ $branchProduct->wholesale ? 'checked' : '' }} class="custom-switch-input">--}}
                                            {{--<span class="custom-switch-indicator"></span>--}}
