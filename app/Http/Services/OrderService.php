...
class OrderService
{
    ...
    public function fillWebRequest(Request $request, $order = null)
    {
        if (!$order) {
            $order = new order();
        }
        ...
        $order->address_id = auth()->user()->addresses()->get()->last()->id;
        $order->status = OrderStatus::SUBMITTED;
        $order->type = OrderTypes::NORMAL;
        $order->payment_type = PaymentTypes::CASH_OND_DELIVERY;
        $order->save();
        ...
        foreach ($products as $product) {
            $branchProduct = BranchProduct::find($product["id"]);
            $branchProductPrice = $branchProduct->finalPriceAfterDiscount;
            $product['price'] = $branchProductPrice * $product['quantity'];
            $product['branch_product_id'] = $product['id'];
            $product['category_id'] = $branchProduct->category_id;
            $product['product_id'] = $branchProduct->product_id;
        ...
        $orderDeliveryFee = $this->getOrderDeliveryFee($order);
        $orderDeliveryCost = $this->getOrderTotalCost($order->id);

        $clientMailable = new CreateOrderClientMail($order, $order->user, $orderDeliveryFee, $orderDeliveryCost);
        $this->orderNotification($order, $vendorTitleNotificationMessage, $clientMailable, $clientNotificationTitleMessage);
